import { Body, ClassSerializerInterceptor, Controller, Get, HttpCode, HttpStatus, Param, Put, Redirect, Render, Res, UseInterceptors } from '@nestjs/common';
...
export class VoteUpController {
  ...
  public async voteDetail(
    ...
    const documnt = await this.VoteModel.findById(new ObjectID(id)).exec();
    if(documnt === null) {
      response.redirect('');
    }
    const vote = plainToClass(Vote, documnt!.toObject());
